...
ticketPriceTotal = runners.reduce((acc, val) => acc + val.donation, runners[0].donation);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - We need the runners who donated at least $275
let bigDonors = runners.filter(runner => runner.donation >= 275);
console.log(bigDonors);

// Problem 2 - We need all of the runners name formatted last name, first name.
let formattedFullNames = runners.map(runner => `${runner.last_name}, ${runner.first_name}`);
console.log(formattedFullNames);

// Problem 3 - We need to know how many company runners have a shirt size of Medium
let mediumShirts = runners.reduce((acc, val) => {
    if(val.shirt_size === "M") {
        return acc + 1;
    }
    return acc;
...
console.log(`${mediumShirts} runners have medium shirts.`);